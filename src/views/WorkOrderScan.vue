      ...
      <div v-if="workOrder" class="work-order-info">
        ...
        <div class="info-grid">
          ...
          <div class="info-item">
            ...
            <span class="info-value">{{ workOrder.customer_id }}</span>
          </div>
          <div class="info-item highlight">
            <label>Order QTY:</label>
            <span class="snp-value">{{ workOrder.order_quantity }}</span>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async parseWorkOrderLabel(rawString) {
      ...
      // ตำแหน่ง 0-8: Line Code (L0501BA1) - ไม่ต้องเก็บแยก
      const lineCode = rawString.substring(0, 8);
      ...
      if (itemCodeMatch) {
        ...
        if (itemPart3Match) {
          result.item_id = `${itemPart1}-${itemPart2}-${itemPart3Match[1]}`; // เก็บเป็น item_id
      ...
      if (customerIdMatch) {
        result.customer_id = customerIdMatch[1];
      }
      ...
      if (orderNoMatch) {
        ...
        // วันที่แรก (20251118) - Completion Date
        const date1 = orderNoMatch[2];
        result.completionDate = `${date1.substring(0, 4)}/${date1.substring(4, 6)}/${date1.substring(6, 8)}`;

        // วันที่สอง (20251118) - Order Date
        const date2 = orderNoMatch[3];
        result.oderDate = `${date2.substring(0, 4)}/${date2.substring(4, 6)}/${date2.substring(6, 8)}`;
      }
